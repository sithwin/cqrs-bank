...
public class AccountEventStore implements EventStore {
  ...
  public List<String> getAggregateIds() {
    ...
    if(eventStream == null || eventStream.isEmpty()) {
      throw new IllegalStateException("Could not retrieve from the event store");
    }
    return eventStream.stream().map(x -> x.getAggregateIdentifier()).distinct().collect(Collectors.toList());
  }
}
